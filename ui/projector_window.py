...
class ConfettiWidget(QWidget):
    """Widget customizado que desenha confetes caindo."""
    def __init__(self, parent=None):
        ...
        # Inicializa 120 partículas espalhadas
        for _ in range(120):
    ...
    def start(self):
        self.timer.start(30) # ~33fps para ser leve e fluido
    ...
    def create_particle(self, initial=False):
        ...
        # Se for inicial, espalha pela tela toda. Se não, nasce no topo.
    ...
    def update_particles(self):
        ...
        for p in self.particles:
            p[0] += p[4] # Movimento horizontal
            p[1] += p[3] # Movimento vertical (gravidade)
            
            # Se saiu por baixo, recria no topo
            if p[1] > h:
                new_p = self.create_particle(initial=False)
                p[0], p[1], p[2], p[3], p[4], p[5] = new_p
                
            # Rebate levemente nas laterais
            if p[0] < 0 or p[0] > w:
                p[4] *= -1
...
class ProjectorWindow(QWidget):
    def __init__(self):
        ...
        # Fundo Animado de Confetes (fica atrás do layout)
        self.confetti = ConfettiWidget(self.idle_container)
        
        # Layout para os textos e logo (fica sobre os confetes)
        ...
        # Sombra no texto para garantir leitura sobre os confetes
        self.idle_text.setStyleSheet("""
            color: #4DA8DA; 
            background-color: transparent;
        """)
        
        # Adiciona um efeito de sombra no texto para destacar dos confetes
    ...
    def show_score(self, score: int):
        ...
        self.score_label.setStyleSheet(f"color: {color}; background-color: #000000;")
